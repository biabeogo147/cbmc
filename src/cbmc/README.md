...
CBMC can now enable modular operating-system API integrations through command
line options owned by this folder:

1. `--os-api osek` selects the OSEK integration.
2. `--osek-oil <file>` supplies the OIL task configuration used by that
   integration.

The parsing and validation logic for those options lives here, while the
runtime scheduler semantics are delegated to `src/os-api/` and `goto-symex/`.
The current regression fixture for that flow is
