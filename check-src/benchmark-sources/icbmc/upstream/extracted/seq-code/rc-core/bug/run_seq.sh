...
fileName='rc_core_bug_por_seq.c'
...
#
#(time $BLAST -alias empty -enable-recursion -noprofile -cref -sv-comp -lattice -include-lattice symb -nosserr -errorpathfile $dataFolder/blast.error -propertyfile $prpFile $fileName &> $dataName'_blast.txt') &>> $timeName'_blast.txt'
#
...
(time $CBMC --unwind 3 --error-label ERROR $fileName &> $dataName'_cbmc.txt') &>> $timeName'_cbmc.txt'
