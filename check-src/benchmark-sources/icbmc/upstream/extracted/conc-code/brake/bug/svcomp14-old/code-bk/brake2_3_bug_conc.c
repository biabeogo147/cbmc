...
int __TRANS_thread_sync[] = {0, 0, 0, 0};

void __VERIFIER_atomic_TRANS_atomic_assume_task_compute(void) {
__VERIFIER_assume(__TRANS_thread_running[1]==0 && __TRANS_thread_running[2]==0 && __TRANS_thread_running[3]==0);
}
...
__VERIFIER_assume(__TRANS_thread_running[2]==0 && __TRANS_thread_running[3]==0);
}
...
__VERIFIER_assume(__TRANS_thread_running[1]==0 && __TRANS_thread_running[3]==0);
}
...
void TRANS_atomic_sync_init_task_compute() {
__TRANS_thread_sync[0]=1;
}

void TRANS_atomic_sync_init_task_RR_Wheel() {
__TRANS_thread_sync[1]=1;
}

void TRANS_atomic_sync_init_task_FL_Wheel() {
__TRANS_thread_sync[2]=1;
}

void TRANS_atomic_sync_init_task_FR_Wheel() {
__TRANS_thread_sync[3]=1;
}

void __VERIFIER_atomic_sync_threads() {
__VERIFIER_assume(__TRANS_thread_sync[0]==1 && __TRANS_thread_sync[1]==1 && __TRANS_thread_sync[2]==1 && __TRANS_thread_sync[3]==1);
}
...
TRANS_atomic_sync_init_task_compute();
__VERIFIER_atomic_end();

__VERIFIER_atomic_sync_threads();
//#endif
...
__VERIFIER_atomic_TRANS_task_compute_enter_thread();
  ...
  // Lihao
  {
    ...
    * brake_acc_nodiv_ctrl_P.vkmh_Gain;
    ...
    if (!(fabs(vkmh-tmp)<=0.001)) irq_err();
...
TRANS_atomic_sync_init_task_RR_Wheel();
__VERIFIER_atomic_end();
...
TRANS_atomic_sync_init_task_FL_Wheel();
__VERIFIER_atomic_end();
...
TRANS_atomic_sync_init_task_FR_Wheel();
__VERIFIER_atomic_end();
...
int_T main(int_T argc, const char *argv[])
{
  ...
  while (1) 
  {
    ...
    pthread_create(&t1, NULL, task_RR_Wheel, NULL);
    pthread_create(&t2, NULL, task_FL_Wheel, NULL);
    pthread_create(&t3, NULL, task_FR_Wheel, NULL);
    pthread_create(&t4, NULL, task_compute, NULL);
